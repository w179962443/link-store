...
- GitHub Gist 可选备份：作为本地数据库之外的额外快照通道。
...
- 如果 GitHub Gist 备份失败，本地保存不会回滚；本地成功始终优先于远程同步成功。
...
应用内提供一个可选的 GitHub Gist 备份开关，用于在每次保存 URL 或导出后，把当前“未导出列表”同步到一个私有 Gist。
...
远程备份文件按 `ISO-8601 时间戳<TAB>URL` 存储，例如：

```text
2026-07-15T08:30:15Z	https://example.com/article
2026-07-15T08:31:02Z	https://news.site/post/123
```

合并逻辑采用秒级时间戳去重：同步时先比较本地和远程的记录条数，以条数更多的一侧作为主集合，再遍历另一侧，把主集合中不存在的秒级 key 合并进来，随后同时更新本地数据库和 GitHub Gist。
...
- `app/src/main/java/com/example/urlkeeper/backup`：GitHub Gist 备份设置与同步实现。
